...
void GrimsonGMM::InitModel(const Image& data)
{
	...
	for(unsigned int i = 0; i < m_params.Size()*m_params.MaxModes(); ++i)
	{
		...
		m_modes[i].mu = std::vector<float>( m_params.Channels(),0);
	}
}

void GrimsonGMM::Update(int frame_num, const Image& data,  const BwImage& update_mask)
{
																	...
																	unsigned char& low_threshold, unsigned char& high_threshold)
{
	...
	for (int iModes=0; iModes < numModes; iModes++)
	{
		...
		if (!bFitsPDF)
		{
			...
			std::vector<float> mu = m_modes[pos].mu;

			std::vector<float> dmu = mu;
			float dist = 0;
			for(unsigned ch = 0; ch < mu.size();++ch){
				dmu[ch] = mu[ch] - pixel(ch);
			...
			if(dist < m_params.LowThreshold()*var)
			{
				...
				for(unsigned ch = 0; ch < mu.size();++ch){
					m_modes[pos].mu[ch] = mu[ch] - k*(dmu[ch]);
				}
				//limit the variance
				float sigmanew = var + k*(dist-var);
				m_modes[pos].variance = sigmanew < m_params.MinVariance() ? m_params.MinVariance() : sigmanew > m_params.VarianceFactor()*m_params.Variance() ? m_params.VarianceFactor()*m_params.Variance() : sigmanew;
	...
	if (!bFitsPDF)
	{
		...
		for(unsigned ch = 0; ch < m_modes[pos].mu.size();++ch){
			m_modes[pos].mu[ch] = pixel.data[ch];
		}
		m_modes[pos].variance = m_params.Variance();
